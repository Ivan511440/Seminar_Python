...
class Formatter:
    def __init__(self, aim_path: str, total_dct: dict = None): # type: ignore
        self._aim_path = aim_path
        self._total_dct = self._walk_to_dir(total_dct)

    def _walk_to_dir(self, total_dct) -> dict[str, dict[str]]: # type: ignore
    ...
    def _format_dict(self, total_dct: dict[str, dict[str]], # type: ignore
                     path: str,
                     item_name: str,
                     item_type: str) -> None:
        if item_type == 'F':
        ...
        for dirpath, dirnames, filenames in os.walk(count_path):
            ...
            if dirnames:
                dir_size += sum([self._count_size(os.path.join(dirpath, subdir))
                                 for subdir in dirnames]) # type: ignore
        return dir_size
